...
export default async (b) => {
	try {
		...
		let p = pList[lib.config.extension_重塑系列削弱版_update_source] || "";
		let m;
		let success = false;
		for (const u of [p, ...pList.filter(x => x !== p)]) {
			try {
				const r = await fetch(`${u}https://raw.githubusercontent.com/rockkkkkk863/rockff/refs/heads/main/manifest.json`);
				if (r.ok) {
		...
		// 读取新增的配置：是否仅更新代码文件
		const onlyCode = lib.config.extension_重塑系列削弱版_update_method;
		...
		for (let i = 0; i < entries.length; i += 5) {
			...
			await Promise.all(batch.map(async ([f, h]) => {
				...
				const path = `extension/重塑系列削弱版/${f}`;
		...
		if (!confirm(`《重塑系列削弱版》发现新版本 ${m.version}\n${updateFiles.length}个文件需更新，是否继续？\n更新说明:\n${m.update || '无'}`)) return

		const prog = createProgress("更新 重塑系列削弱版 扩展", updateFiles.length);
		game.importedPack = true;
		...
		try {
			for (let i = 0; i < updateFiles.length; i++) {
				...
				const r = await fetch(`${p}https://raw.githubusercontent.com/rockkkkkk863/rockff/refs/heads/main/${f}`);
				if (!r.ok) throw new Error(`下载失败: ${f}`);

				const data = await r.arrayBuffer();
				const fullPath = `extension/重塑系列削弱版/${f}`;
				const dir = fullPath.split("/").slice(0, -1).join("/");

				await game.promises.createDir(dir);
				await game.promises.writeFile(data, dir, fullPath.split("/").pop());
			}
			await game.promises.writeFile(JSON.stringify(m, null, 2), "extension/重塑系列削弱版", "manifest.json");
			...
			const f = (await clean("extension/重塑系列削弱版")).filter(file => {
			...
			if (f.length) {
				...
				for (let i = 0; i < f.length; i++) {
					...
					await game.promises.removeFile(`extension/重塑系列削弱版/${f[i]}`)
				}
				p.remove();
			}
